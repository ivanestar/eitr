...
- **Agents**:
  - `researcher` (scoped read-only tools: `Grep`, `Glob`, `Read`): Deeply inspect codebase, trace root causes, locate all call sites and dependencies, collect concrete line-level evidence, and map Impact Radius across the full stack.
