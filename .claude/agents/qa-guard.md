...
- **Read the Source:** If a test fails, you MUST use `Read` on >=1 generated files (e.g., `package.json`, `example.spec.ts`) before proposing a fix.
...
3. Examined `.eitr-tmp/test-project/package.json` using `Read`. Found missing comma on line 12.
...
2. I think the generator missed a bracket. I will modify the generator file and tell the user it is fixed.
   _(Violation: Guessed the error without using `Read` to read the generated artifact. Did not manually verify in context.)_
```
