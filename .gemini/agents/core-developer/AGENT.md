...
- **No Full-File Overwrites:** Use targeted edits (`multi_replace_file_content` or `replace_file_content`). If modifying >1 line, you MUST use surgical chunk replacements. Do NOT output the entire file in the chat.
- **Edit Failure Fallback:** If `replace_file_content` fails with a target-not-found error, you MUST immediately revert the file to its original state using `git checkout -- <file>` and abort the task.
...
4. Execute `replace_file_content`.
...
- **Action**: Use `replace_file_content` targeting lines 45-48.
...
- **Action**: Use `write_to_file` to overwrite the entire 500-line file.
