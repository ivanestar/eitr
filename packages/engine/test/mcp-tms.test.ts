...
import { planAiTmsSkills } from '../src/plan/templates/ai-tms-skills.js';
...
describe('MCP TMS & AI-First Subsystem Generators', () => {
  it('returns empty array when tmsProvider is none or omitted', () => {
    ...
    expect(planAiTmsSkills('none')).toEqual([]);
  ...
  it('generates standalone MCP server files for azure-devops', () => {
    ...
    expect(paths).toContain('.mcp/tms-bridge/cli.js');
  ...
  it('generates 7 specialized SDET agents for all supported assistants (Gemini, Claude, Cursor, Windsurf, Codex, Copilot)', () => {
    const files = planAiAgents(['gemini', 'claude', 'cursor', 'windsurf', 'codex', 'copilot']);
    expect(files.length).toBe(42); // 7 agents * 6 assistants
    const paths = files.map((f) => f.path);
    ...
    expect(paths).toContain('.gemini/agents/sdet-architect/agent.md');
    expect(paths).toContain('.gemini/agents/pom-engineer/agent.md');
    expect(paths).toContain('.gemini/agents/test-automator/agent.md');
    ...
    expect(paths).toContain('.cursor/rules/agent-tms-validator.mdc');
    expect(paths).toContain('.cursor/rules/agent-pom-engineer.mdc');
    ...
    expect(paths).toContain('.codex/agents/tms-validator.md');
    expect(paths).toContain('.github/agents/tms-validator.md');

    const tmsValidator = files.find((f) => f.path === '.gemini/agents/tms-validator/agent.md');
    ...
    const orchestrator = files.find((f) => f.path === '.gemini/agents/sdet-orchestrator/agent.md');
    expect(orchestrator?.source.text).toContain('Mandatory Execution Quality Gate');
    expect(orchestrator?.source.text).toContain('test:sanity');
    expect(orchestrator?.source.text).toContain('tms-validator');

    const architect = files.find((f) => f.path === '.gemini/agents/sdet-architect/agent.md');
    expect(architect?.source.text).toContain('Dependency Injection');
    expect(architect?.source.text).toContain('test.extend');
    expect(architect?.source.text).toContain('Mandatory POM Sanity Test Coverage');

    const pomEngineer = files.find((f) => f.path === '.gemini/agents/pom-engineer/agent.md');
    ...
    expect(pomEngineer?.source.text).toContain('MANDATORY AUTONOMOUS TEST EXECUTION');
    ...
    const automator = files.find((f) => f.path === '.gemini/agents/test-automator/agent.md');
    expect(automator?.source.text).toContain('Dynamic Test Data Management');
    expect(automator?.source.text).toContain('test.step');

    const auditor = files.find((f) => f.path === '.gemini/agents/assertion-auditor/agent.md');
    ...
    const traceDebugger = files.find((f) => f.path === '.gemini/agents/trace-debugger/agent.md');
  ...
  it('generates 6 operational skills for all supported AI assistants', () => {
    ...
    expect(files.length).toBe(42); // 6 skills * 5 + (6 prompts + 6 skills for copilot)
    const paths = files.map((f) => f.path);

    expect(paths).toContain('.gemini/skills/auth-bootstrap/SKILL.md');
    expect(paths).toContain('.gemini/skills/scan-and-generate-pom/SKILL.md');
    ...
    expect(paths).toContain('.gemini/skills/map-site/SKILL.md');
    ...
    const mapSkill = files.find((f) => f.path === '.gemini/skills/map-site/SKILL.md');
    ...
    const pomSkill = files.find((f) => f.path === '.gemini/skills/scan-and-generate-pom/SKILL.md');
    expect(pomSkill?.source.text).toContain('tests/pom-sanity');
    ...
    const automateSkill = files.find((f) => f.path === '.gemini/skills/automate-ticket/SKILL.md');
    ...
    const healSkill = files.find((f) => f.path === '.gemini/skills/heal-test/SKILL.md');
  ...
  it('includes MCP files, AI agents, and operational skills by DEFAULT with zero options', () => {
    ...
    expect(paths).toContain('.gemini/agents/sdet-orchestrator/agent.md');
    expect(paths).toContain('.gemini/agents/pom-engineer/agent.md');
    expect(paths).toContain('.cursor/rules/agent-sdet-orchestrator.mdc');
    expect(paths).toContain('.claude/agents/sdet-orchestrator.md');
    expect(paths).toContain('.windsurf/rules/agent-sdet-orchestrator.md');
    expect(paths).toContain('.codex/agents/sdet-orchestrator.md');
    expect(paths).toContain('.github/agents/sdet-orchestrator.md');
    ...
    expect(paths).not.toContain('.cursor/rules/harmonize.mdc');
    expect(paths).not.toContain('.windsurf/rules/harmonize.md');
    expect(paths).not.toContain('.gemini/skills/framework-harmonizer/SKILL.md');
