...
import { frozenDescribe, frozenIt } from './helpers/frozen.js';
...
describe('cicd.ts content assertions', () => {
  // ── GitLab CI ──────────────────────────────────────────────────────────
  frozenIt('GitLab CI python branch: pip-audit + anti-dup rule', () => {
  ...
  frozenIt('GitLab CI csharp branch: dotnet vulnerability audit + anti-dup rule', () => {
    const yaml = renderGitlabCi('csharp');
    expect(yaml).toContain('dotnet list package --vulnerable --include-transitive');
    expect(yaml).toContain('CI_OPEN_MERGE_REQUESTS');
  });

  frozenIt('GitLab CI java-maven branch: mvn test, not gradle test', () => {
  ...
  frozenIt('GitLab CI java-gradle branch: gradle test, not mvn test', () => {
  ...
  // ── Jenkinsfile ────────────────────────────────────────────────────────
  frozenIt('Jenkinsfile python branch: pytest-split shard matrix + pip-audit', () => {
  ...
  frozenIt('Jenkinsfile csharp branch: dotnet test + vulnerability audit', () => {
  ...
  frozenIt('Jenkinsfile java-maven branch: mvn test, not gradle test', () => {
    const jf = renderJenkinsfile('java', 'maven');
    expect(jf).toContain('mvn test');
    expect(jf).not.toContain('gradle test');
  });

  frozenIt('Jenkinsfile java-gradle branch: gradle test, not mvn test', () => {
  ...
  // ── GitHub Actions ─────────────────────────────────────────────────────
  frozenIt('GitHub Actions python branch: pip cache + pip-audit', () => {
    const yaml = renderGithubActions('python');
    expect(yaml).toContain(`cache: 'pip'`);
    expect(yaml).toContain('pip install pip-audit && pip-audit');
  });

  frozenIt('GitHub Actions java-maven branch: maven cache + mvn test', () => {
  ...
  frozenIt('GitHub Actions java-gradle branch: gradle cache + sharded gradle test dispatch', () => {
  ...
  frozenDescribe(
    'GitHub Actions C#/Java: 4-way FNV-1a shard matrix + indexed artifacts (Track 8)',
    () => {
      ...
      frozenIt(
        'the FNV-1a partition function is byte-for-byte identical across csharp/maven/gradle branches',
        () => {
  ...
  describe('GitHub Actions YAML is syntactically valid', () => {
    ...
    frozenIt('cypress branch parses and has jobs', () => {
  ...
  describe('GitLab CI YAML is syntactically valid', () => {
    ...
    frozenIt('cypress branch parses and has stages', () => {
  ...
  frozenIt(
    'TeamCity Kotlin DSL python branch imports matrix correctly (not buildFeatures.matrix)',
    () => {
