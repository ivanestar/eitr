...
export function renderCypressPackageJson(projectName: string, isTs: boolean): string {
  ...
  "scripts": {
    "test": "cypress run",
    "test:sanity": "cypress run --spec 'cypress/e2e/pom-sanity/**/*.sanity.spec.*'",
...
/** cypress/e2e/cpom-showcase.cy.ts or .js */
export function renderCypressCpomShowcase(isTs: boolean): string {
  const baseImport = isTs
    ? '../../components/base/base-page'
    : '../../components/base/base-page.js';
  const primImport = isTs ? '../../components/primitives' : '../../components/primitives/index.js';
  const widgetImport = isTs ? '../../components/widgets' : '../../components/widgets/index.js';

  if (isTs) {
    return `import { BasePage } from '${baseImport}';
import { TextInput, NativeSelect, Link } from '${primImport}';
import { Table, Dialog } from '${widgetImport}';
...
class ShowcaseScreen extends BasePage {
  readonly path = '/';
  readonly search = new TextInput('input[aria-label="Search"]');
  readonly role = new NativeSelect('select[aria-label="Role filter"]');
  readonly next = new Link('a:contains("Next")');
  readonly people = new Table('table');
  readonly dialog = new Dialog('div[role="dialog"]');
}

describe('CPOM Showcase (Cypress)', () => {
  beforeEach(() => {
    cy.document().then((doc) => {
  ...
  it('a page composes components; a table nests rows', () => {
    const screen = new ShowcaseScreen();
    screen.people.rows().should('have.length', 2);
  });

  it('universal actions and primitive controls', () => {
    const screen = new ShowcaseScreen();
    screen.search.fill('ada');
    screen.search.valueNow().should('eq', 'ada');

    screen.role.selectOption('admiral');
    screen.role.getSelectedValue().should('eq', 'admiral');
  });
});
`;
  }
...
class ShowcaseScreen extends BasePage {
  constructor() {
    super('/');
    this.search = new TextInput('input[aria-label="Search"]');
    this.role = new NativeSelect('select[aria-label="Role filter"]');
    this.next = new Link('a:contains("Next")');
    this.people = new Table('table');
    this.dialog = new Dialog('div[role="dialog"]');
  }
}
...
/** cypress/e2e/api-showcase.cy.ts or .js */
export function renderCypressApiShowcase(isTs = true): string {
  const clientImport = isTs ? '../../shared/utils/api-client' : '../../shared/utils/api-client.js';
  return `import { ApiClient } from '${clientImport}';

describe('API Client Showcase (Cypress)', () => {
  const api = new ApiClient();

  it('makes REST GET request via ApiClient', () => {
