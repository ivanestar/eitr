...
2. **Concurrent Route Exploration & Pagination Normalization (Worker Pool):**
   ...
   - **\`scripts/crawl-budget.mjs\` owns the frontier. Do not canonicalize URLs, apply bounds, or decide what to skip yourself.** Open the pass with \`node scripts/crawl-budget.mjs start --base-url="<url>" [--role=<slug>]\`, then ask it about every single candidate link before touching it:
     \`\`\`
     node scripts/crawl-budget.mjs check --url="<href>" --depth=<hops-from-start>
     \`\`\`
     ...
     After the navigation completes, report it back: \`node scripts/crawl-budget.mjs visited --url="<href>" --status=<http status> --content-type="<response content-type>"\`. A \`keep: false\` answer means the response was not a page at all (an extensionless download or export endpoint) - discard that route rather than adding it to the site map.
   - **Commit each route before moving to the next one. Never hold the route set in memory until the end.** Open the journal once, before the first navigation: \`node scripts/artifact-journal.mjs begin --stage=site-map\`. Then, the moment a route's own extraction is finished (title, regions, components, contentHash, httpStatus, screenshot, visualTriage - everything Step 3a would put in its entry), write that entry to a temporary file and commit it:
     \`\`\`
   ...
   - Traversal bounds (max depth, max pages, max concrete URLs per canonical template) are enforced by \`scripts/crawl-budget.mjs\`, not by you - run \`start\` once per role pass so each pass gets its own budget. The one bound the script cannot see is scrolling, because it never touches the page: limit live exploration to a maximum of 2 viewport scrolls per route.
   - When the pass ends, run \`node scripts/crawl-budget.mjs report\`. Its \`coverage\` field is either \`null\` (the crawl exhausted every discoverable link) or an object ready to copy verbatim into \`site-map.json\`'s own \`coverage\` - use what it returns rather than judging completeness yourself. Its \`skipped\` histogram is worth one line in the run summary when \`max-per-template\` or \`max-pages\` appears in it: that is the crawler telling you the application has more of something than the map now shows.
