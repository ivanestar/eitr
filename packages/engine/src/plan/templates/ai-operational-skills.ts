...
6. **Summary (no blocking gate):**
   * Print a short summary - how many journeys were drafted, how many conditions landed at each test level - and point at \`artifacts/test-cases/test-cases.json\` for review "anytime." Do not ask for approval before finishing: this stage's draft is deliberately reviewable-later, not gate-blocking, a departure specific to this stage only - every earlier stage in this pipeline keeps its own blocking Human Sign-Off Gateway unchanged.
`,
    },
    {
...
2. **Stage Loop (Guided mode; repeats once per pipeline stage):**
   ...
   * **Exception for \`/design-test-cases\`:** it has no blocking Human Sign-Off Gateway of its own - it writes \`artifacts/test-cases/test-cases.json\` and moves on. For this stage only, skip the Approve/Reject loop entirely and go straight to "What next?" below.
...
4. **End of Chain:**
   ...
   * If the stage is \`test-cases-drafted\`: tell the user plainly that test cases are drafted in \`artifacts/test-cases/test-cases.json\`, and that running \`/automate-ticket\` with no ticket ID will automate them directly - no TMS ticket required.
