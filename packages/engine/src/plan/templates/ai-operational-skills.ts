...
6. **Business-Intent & Criticality Analysis (Automatic, Strictly Read-Only):**
   - Runs automatically for every active route, as part of both \`create\` and \`update\`, immediately after Step 4 - no separate request needed, unless the user explicitly asked to skip it (e.g. "just map the site, skip business-intent"):
     ...
     * **\`reasoning\`** (both \`businessFeature\` and \`criticalityTier\`): a short, non-empty string naming which checklist criterion or confidence-rule branch this value matched (e.g. "Matches critical checklist: heading text names a checkout/payment flow") - never a restatement of the evidence excerpt in different words; the validator rejects a \`reasoning\` that exactly equals its own evidence excerpt.
     ...
     * **Self-verification pass (before the Mechanical Gate, zero new tooling)**: for every entry just drafted or changed, re-check its \`criticalityTier.value\` against the checklist above - does the cited evidence actually match the claimed bucket's criteria? If not, downgrade it to the tier the evidence genuinely supports before writing the file. This is a checklist-conformance check, not a substitute for the Human Sign-Off Gateway below, which stays mandatory and unchanged - it exists to catch the most obvious rubric mismatches before a human has to.
   ...
   - **Human Sign-Off Gateway:** present a Business-Intent Review Artifact as one labeled block per new/changed entry, never a Markdown table - a wide table with variable-length Cyrillic/Unicode text and long evidence excerpts renders unreadably in a plain terminal, which has no Markdown rendering. Resolve each \`routeId\` to its \`docs/site-map/site-map.json\` path/title first - never show the raw \`routeId\` to the human. Per route: one heading line with the resolved path and title, then \`businessFeature\` and \`criticalityTier\` each get their own 3-line block in that order - \`businessFeature\`'s summary line is \`Feature: <value>  Confidence: <confidence>\`, \`criticalityTier\`'s is \`Route criticality: <value>  Confidence: <confidence>\` - never share one \`Confidence:\` slot between them, since each is computed independently from its own evidence and routinely disagrees (e.g. a route's business-feature label can be \`high\`-confidence while its criticality assignment is only \`low\`-confidence, or vice versa). Each summary line is followed by that Field<T>'s own \`Why: <reasoning>\` line, then one \`Evidence (<signal>): "<excerpt>"\` line per evidence entry it carries. State explicitly: this file is NOT authoritative until a human has reviewed it - no other skill or agent should treat an entry with \`reviewed: false\` as ground truth. Once the human actually approves an entry in conversation, set that entry's \`reviewed\` to \`true\` and \`reviewedBy\` to \`'human'\` in \`docs/analysis/business-intent.json\` before continuing - never set \`reviewed: true\` without also setting \`reviewedBy\`.
