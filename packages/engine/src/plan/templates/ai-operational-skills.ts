...
function withLanguageDefaultNote(skill: SkillDefinition): SkillDefinition {
  const [heading, ...rest] = skill.content.split('\n\n');
  return { ...skill, content: `${heading}\n\n${LANGUAGE_DEFAULT_NOTE}\n\n${rest.join('\n\n')}` };
}
...
export function planAiOperationalSkills(
  ...
  const skills = buildOperationalSkills(automationTool, language).map(withLanguageDefaultNote);
