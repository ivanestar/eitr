...
// Who actually runs the capture command, and why. Printing it for the human is the default, not a
// fallback: the previous wording gated that on whether the assistant could execute a terminal
// command at all, which is the wrong test and was live-observed failing - essentially every
// assistant CAN execute the command, so the fallback never triggered, while the browser window it
// launched never reached the user. Two independent failure modes, either sufficient on its own: the
// assistant cannot verify a GUI window it spawns is visible on the human's screen (sandboxed,
// remote, or display-less execution), and `codegen`/`open --save-storage` block the terminal for the
// entire human-paced login. `@playwright/cli` (verified 2026-09-07: package `@playwright/cli`
// v0.1.19, binary `playwright-cli`; the older standalone `playwright-cli` package is deprecated in
// favor of it) removes only the blocking half via its persistent-session model - `open --headed`
// returns in ~2.5s leaving a live session, then `state-save`/`close` - so it is offered as the
// agent-driven path, never as a reason to skip the visibility confirmation.
const AUTH_SETUP_CAPTURE_POLICY = `- **Default: print the command and let the human run it in their own terminal.** Name which role is being captured right now - one role, one command, in the order Step 3 settled, never every role's command dumped at once. Two independent reasons, either enough on its own: you cannot verify that a browser window YOU launch actually reaches the human's screen (a sandboxed, remote, or display-less execution environment produces no visible window and no error either), and the command blocks for the whole login, which is human-paced, not machine-paced.
- **Run it yourself only when both are true**: the human explicitly confirmed they can see windows you open, and you can hold a minutes-long foreground process without timing out. Ask rather than assume - "can you see a browser window if I open one from here?" is one short question, and getting it wrong wastes the entire capture round on a window nobody ever saw.
- **If you do drive it yourself, use the non-blocking path** (\`@playwright/cli\` - npm package \`@playwright/cli\`, binary \`playwright-cli\`): \`npx playwright-cli open --headed "<url>"\` returns in about two seconds and leaves a live named browser session instead of blocking, so you stay responsive in chat while the human logs in; when they say they are done, run \`npx playwright-cli state-save "<session file>"\` then \`npx playwright-cli close\`. Create the \`.auth/\` directory first - \`state-save\` does not create parent directories and fails with ENOENT when it is missing. This fixes the blocking half only; it does nothing about whether a display exists, so it never replaces the confirmation above.`;
...
- State plainly: "A real browser window will open at <url>. Log in there yourself - any SSO/MFA/2FA, just do it normally, that's the whole point of using a real browser and a real human. Close the window once you see you're logged in - the session saves automatically."
- **The command for this role**, using this project's own CLI wrapper (it resolves the URL exactly as described above and writes that role's session file): \`npx eitr auth --role <role>\`. The one-kind-of-user case from Step 3 is plain \`npx eitr auth\`, which writes \`.auth/user.json\`. Without this project's CLI installed, the direct equivalent is \`npx playwright open --save-storage=".auth/<role>.json" "<url>"\`. Never print, ask for, or repeat back the username or password itself - the human types their credentials into the browser, and no part of this flow needs you to know them.
${AUTH_SETUP_CAPTURE_POLICY}
- **Mechanically verify, never assume success**: after the command exits, confirm the output file exists, is non-empty JSON, and actually contains at least one cookie or \`origins\` entry. If not: "Doesn't look like login actually completed - no session data was captured. Try again?" - a file that merely exists is not proof of a real session.
${AUTH_SETUP_NETWORK_OBSERVATION_BULLET}
