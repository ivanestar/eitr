...
6. **Business-Intent & Criticality Analysis (Automatic, Strictly Read-Only):**
   - Runs automatically for every active route, as part of both \`create\` and \`update\`, immediately after Step 4 - no separate request needed, unless the user explicitly asked to skip it (e.g. "just map the site, skip business-intent"):
     ...
     * **Criticality Re-Derivation (using the now-confirmed purpose and roles, before the Mechanical Gate)**: re-examine every route's \`criticalityTier\` against two more criteria alongside the checklist above. (a) Does this route's functionality directly deliver \`corePurpose.selected.value\`? If the checklist alone placed a route at \`medium\` or \`low\` and it genuinely delivers the confirmed core purpose, raise it to \`high\` - never automatically to \`critical\`, which stays reserved for its own payment/auth/destructive-action criteria regardless of purpose alignment. (b) Is this route reachable by some roles and not others? A real permission boundary makes the route at least \`high\`, because a broken boundary exposes one role's functionality to another. When either raises a tier, rewrite that entry's \`reasoning\` to say so in plain language (e.g. "Only the admin role reached this route; the customer role was redirected to login, so a broken check here would expose administrative functionality.").
       - **(c) The one case where the confirmed purpose LOWERS a tier: a demo/practice application.** The criticality checklist fires on keywords (\`login\`, \`password\`, \`2fa\`, \`secure\`, ...) because on a real application those almost always mark a genuine security surface. On an application whose confirmed \`corePurpose.selected.value\` explicitly describes it as a sandbox, practice, demo, playground, training, tutorial, example, or kata application, those same routes are usually exhibits of the pattern rather than a real credential surface - live-observed: a UI-automation practice sandbox got six routes at \`critical\` for "credential lifecycle", all with the identical reasoning, when nothing real was behind any of them. Only when the human's own confirmed purpose says so, drop such a route from \`critical\` to \`high\` - never lower than \`high\`, and only when ALL of these hold: the route reached \`critical\` through the auth-lifecycle keyword branch alone; no real user data, payment, or irreversible action is actually behind it; and the route is not the application's own real sign-in (a sandbox still has one real login protecting the rest, and that one stays \`critical\`). A payment/checkout/billing keyword match never moves, purpose notwithstanding. When this fires, say so plainly in the \`reasoning\` ("this is a demonstration of an auth pattern in a practice sandbox, not a real credential surface") so the human can overturn it at sign-off - which is the whole point of surfacing it rather than silently keeping either tier.
       - **Nothing else lowers a tier.** Absent an explicitly-confirmed demo/practice purpose, keyword-driven \`critical\` stands: over-testing a route that turned out to be a demo costs some wasted test volume, under-testing a real credential surface costs an incident, and those are not the same mistake.
