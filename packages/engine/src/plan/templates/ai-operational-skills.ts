...
- If you can run a terminal command yourself (true for essentially every assistant this project supports), just run it: \`playwright codegen --save-storage=.auth/user.json <url>\` (a different filename per role, e.g. \`.auth/admin.json\`) - this always opens a real, visible browser window, independent of any MCP configuration. Only if you genuinely have no way to execute a terminal command in this environment, print the exact command above for the human to run themselves, naming the current role being captured (never dump every role's command at once - one role, one command, in the same order Step 3's loop is already going through them).
...
- State plainly: "A real browser window will open at <url>. Log in there yourself - any SSO/MFA/2FA, just do it normally, that's the whole point of using a real browser and a real human. Close the window once you see you're logged in - the session saves automatically."
- If you can run a terminal command yourself (true for essentially every assistant this project supports), just run it: Windows: \`pwsh bin/Debug/net8.0/playwright.ps1 codegen --save-storage=.auth/user.json <url>\`; macOS/Linux: \`bin/Debug/net8.0/playwright.sh codegen --save-storage=.auth/user.json <url>\` (a different filename per role, e.g. \`.auth/admin.json\`) - this always opens a real, visible browser window, independent of any MCP configuration. Only if you genuinely have no way to execute a terminal command in this environment, print the exact command for the human to run themselves, naming the current role being captured (never dump every role's command at once - one role, one command, in the same order Step 3's loop is already going through them).
- **Mechanically verify, never assume success**: after the command exits, confirm the output file exists, is non-empty JSON, and actually contains at least one cookie or \`origins\` entry. If not: "Doesn't look like login actually completed - no session data was captured. Try again?" - a file that merely exists is not proof of a real session.
${AUTH_SETUP_NETWORK_OBSERVATION_BULLET}
...
- State plainly: "A real browser window will open at <url>. Log in there yourself - any SSO/MFA/2FA, just do it normally, that's the whole point of using a real browser and a real human. Close the window once you see you're logged in - the session saves automatically."
- If you can run a terminal command yourself (true for essentially every assistant this project supports), just run it: ${captureCmd} (\`.auth/user.json\` by default, a different filename per role, e.g. \`.auth/admin.json\`) - this always opens a real, visible browser window, independent of any MCP configuration. Only if you genuinely have no way to execute a terminal command in this environment, print the exact command for the human to run themselves, naming the current role being captured (never dump every role's command at once - one role, one command, in the same order Step 3's loop is already going through them).
- **Mechanically verify, never assume success**: after the command exits, confirm the output file exists, is non-empty JSON, and actually contains at least one cookie or \`origins\` entry. If not: "Doesn't look like login actually completed - no session data was captured. Try again?" - a file that merely exists is not proof of a real session.
${AUTH_SETUP_NETWORK_OBSERVATION_BULLET}
...
- State plainly: "A real browser window will open at <url>. Log in there yourself - any SSO/MFA/2FA, just do it normally, that's the whole point of using a real browser and a real human. Close the window once you see you're logged in - the session saves automatically."
- If you can run a terminal command yourself (true for essentially every assistant this project supports), just run it: \`npx playwright codegen --save-storage=.auth/user.json <url>\` (a different filename per role, e.g. \`.auth/admin.json\`) - this always opens a real, visible browser window, independent of any MCP configuration. Only if you genuinely have no way to execute a terminal command in this environment, print the exact command above for the human to run themselves, naming the current role being captured (never dump every role's command at once - one role, one command, in the same order Step 3's loop is already going through them).
- **Mechanically verify, never assume success**: after the command exits, confirm the output file exists, is non-empty JSON, and actually contains at least one cookie or \`origins\` entry. If not: "Doesn't look like login actually completed - no session data was captured. Try again?" - a file that merely exists is not proof of a real session.
${AUTH_SETUP_NETWORK_OBSERVATION_BULLET}
...
9. **Final Report:**
   ...
   - **Standalone invocation only** (skip entirely when this run is part of \`/ground-zero-setup\`'s own chain, which already runs this once at its own End of Chain): if this run touched more than one Page Object, dispatch exactly one \`pom-engineer\` subagent for its own Cross-Page-Object Consolidation Pass over the files this run touched - a single bounded pass, never repeated within the same run. A run that touched exactly one Page Object has nothing to consolidate against, skip it.
`,
    },
...
## Workflow
0. **Standalone Entry Check (skip entirely when invoked as part of \`/ground-zero-setup\`'s own chain - its Pre-Flight and per-stage gate already cover this):** state in one plain sentence what this does - "Crawls your live app, builds a visual sitemap, and figures out what each page is for, for you to review." - then ask: **Continue, or stop here?** ${INTERACTIVE_CHOICE_NOTE} Stop on anything but an explicit yes; nothing has run yet at this point.
...
## Workflow
0. **Standalone Entry Check (skip entirely when invoked as part of \`/ground-zero-setup\`'s own chain - its Pre-Flight and per-stage gate already cover this):** state in one plain sentence what this does - "Turns your reviewed test conditions into concrete, readable test cases, for you to review." - then ask: **Continue, or stop here?** ${INTERACTIVE_CHOICE_NOTE} Stop on anything but an explicit yes; nothing has run yet at this point.
1. **Preconditions:**
...
4. **End of Chain (reached once the stage is \`complete\`):**
   ...
   * **Only when \`/automate-test\` actually ran and produced at least one new/updated Page Object this session**: dispatch exactly one \`pom-engineer\` subagent for its own Cross-Page-Object Consolidation Pass (see that agent's own system prompt for the bounded scope and stopping rule) over the files this session touched - a single pass, run once, here, never repeated later in the same run and never expanded into a full-repo audit unless the human explicitly asks for one separately.
