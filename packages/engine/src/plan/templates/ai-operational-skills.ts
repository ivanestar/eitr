...
2. **Parameter & Partition Extraction (Read-Only, With One Narrow Reveal Exception):**
   ...
   * Synthesize 2–3 route-level invariant conditions across the 9 closed negative categories (\`invalid_input\`, \`boundary\`, \`missing_precondition\`, \`concurrent_conflict\`, \`state_violation\`, \`permission_denied\`, \`external_failure\`, \`data_integrity\`, \`error_path\`) into \`conditions[]\` per route entry. Each invariant condition has \`technique: 'architectural-invariant'\`, \`scenario: 'negative'\`, a valid \`negativeCategory\`, \`parameters: {}\`, an illustrative \`description\` (e.g. \`Verify unauthenticated access is redirected to login\`), \`isSpeculative: true\`, \`reviewed: false\`, and an empty \`verification: {}\`.
      - **Negative categories semantic guidance:**
        ...
        - \`invalid_input\` and \`boundary\`: payload and length edge cases not already captured by field partitions.
