...
6. **Assertion Audit & Side-Effect Verification:**
   - Audit test with \`assertion-auditor\` to eliminate fake-green patterns and verify UI + API dual-layer assertions - explicitly including the content-fidelity check above: flag any step whose assertion doesn't reference something the step's own action actually touched.
...
2. **Parameter & Partition Extraction (Read-Only, With One Narrow Reveal Exception):**
   ...
   * Write \`artifacts/analysis/test-conditions.json\` (\`schemaVersion: 1\`) with \`conditions: []\` and \`unsatisfiedPairs: []\` left empty for every new/changed entry.
...
6. **Test-Cases Review Artifact (informational, never blocking):**
   ...
   * If at least one TMS/task-tracker provider is configured (the \`mcp__tms__*\` tools are present in this conversation), ask one short, easily-skippable question naming it: "<Provider> is configured - want these also recorded there as test cases?" ${INTERACTIVE_CHOICE_NOTE} A "no," or no answer at all, is a completely normal outcome here - never re-ask automatically on a later run just because it went unanswered once.
...
## Purpose
A thin orchestrator for a brand-new application, not a new analysis engine of its own: it sequences the currently-built stages of the app-analysis pipeline (\`/map-site create\`, including its automatic Step 6 business-intent inference, then \`/define-test-conditions\`, then \`/design-test-cases\`) so a user does not have to remember which command follows which - while never removing the human decision points those underlying skills already require. It adds zero duplicated crawling, inference, or generation logic; every actual decision about what stage comes next is read from \`scripts/pipeline-status.mjs\`, never hardcoded here, so a future pipeline stage only ever requires extending that one script, not rewriting this skill's own sequencing. This orchestrator stops short of \`/automate-test\` on purpose - unlike the three analysis/drafting stages above, it synthesizes and executes real code, so triggering it is left to the user's own explicit command, never chained automatically even in auto-pilot mode.
...
1. **Pre-Flight Confirmation (mandatory, before anything runs):**
   ...
   * Present, in conversation, and wait for an explicit response before proceeding:
     - **What will run, in order**, starting from the current stage: \`/map-site create\` (full site crawl plus automatic business-intent inference), then \`/define-test-conditions\` (test analysis - defines test conditions for every reviewed route), then \`/design-test-cases\` (test design - test-level classification and a drafted test case per journey). Name \`/automate-test\` (Stage 4) too, as the pipeline's own next step once the above finish - never run by this orchestrator itself (see Purpose), but worth knowing about upfront rather than only discovering it at the very end.
   ...
   * Print \`pipeline-status.mjs\`'s \`roadmap\` field so the human sees the full pipeline shape and exactly where this run starts, before answering anything below.
...
2. **Stage Loop (Guided mode; repeats once per pipeline stage):**
   ...
   * Present that stage's own existing Human Sign-Off Gateway (or, for \`/design-test-cases\`, its Test-Cases Review Artifact) exactly as its own skill defines it, preceded by the current \`roadmap\` from \`pipeline-status.mjs\` so the human always sees stage position alongside the content - this skill does not invent a different review format or shorten the one that already exists.
   * **Exception for \`/design-test-cases\`:** its Test-Cases Review Artifact and optional TMS-recording question (both defined in its own Step 6) are presented exactly as that skill defines them, but there is no blocking Approve/Reject loop for it - go straight to "What next?" below once its own step 6 finishes.
   * **Approve / Reject loop** (every stage except \`/design-test-cases\`, per the exception above): ask the user to approve, or reject with comments. ${INTERACTIVE_CHOICE_NOTE}
   ...
   * **"What next?"**: run \`node scripts/pipeline-status.mjs\` again and read its \`nextCommand\`.
     - **If \`nextCommand\` is anything other than \`/automate-test\`** (i.e. another local-analysis stage remains): ask the user, with a clearly-marked recommended default, whether to continue to it (name it explicitly - the recommended choice), stop here for now (the project is left in a valid, resumable state; resume later via \`nextCommand\` directly or by re-invoking \`/ground-zero-setup\`, which always resumes from whatever \`pipeline-status.mjs\` currently reports rather than restarting), or give an explicit free-text instruction instead of either. ${INTERACTIVE_CHOICE_NOTE}
     - **If \`nextCommand\` is \`/automate-test\`**: this orchestrator's own local-analysis work is done - go straight to End of Chain below instead of asking "continue?", since this orchestrator can inform about that stage but never runs it itself (see Purpose).
3. **Auto-pilot mode:**
   * Skip step 2's pause-and-ask for LOCAL artifact review only - run each stage, then instead of pausing, autonomously approve every new/changed entry from that stage by setting \`reviewed: true\` and \`reviewedBy: 'auto-pilot'\` on it (for \`/design-test-cases\`, still present its Test-Cases Review Artifact and TMS-recording question exactly as that skill defines them - "skip pause-and-ask" governs the approval gate, not whether informational content is shown), then immediately continue to the next stage per \`pipeline-status.mjs\`'s \`nextCommand\`, without asking "what next?" at each stage - unless \`nextCommand\` is \`/automate-test\`, in which case go to End of Chain below regardless of mode, the same trigger condition Guided mode's own "What next?" step uses; this bullet governs local-artifact-review stages only, never code generation.
   * The blanket pre-authorization given at the Pre-Flight Confirmation screen covers only this: proceeding through this pipeline's own local file writes. It does NOT extend to any action with a real external side effect outside this project's own local files - the Test-Cases Review Artifact's own TMS-recording question (if asked) still requires its own explicit answer even in auto-pilot, since writing to an external TMS is exactly this kind of side-effecting action, never covered by local-review pre-authorization.
4. **End of Chain (reached once \`nextCommand\` is \`/automate-test\`, or stage is \`complete\`):**
   * This is never a silent stop - the human already knows they will land here from the Pre-Flight roadmap and, in Guided mode, from "What next?" naming \`/automate-test\` as the recommended next step; End of Chain is where that gets acted on, not a surprise dead end.
   * Print the current \`roadmap\` so the human sees exactly how much of the pipeline is done.
   * If the stage is \`test-cases-drafted\`: tell the user plainly that test cases are drafted in \`artifacts/test-cases/test-cases.json\` (and, if asked and answered, already recorded in their TMS per \`/design-test-cases\`'s own question), and that running \`/automate-test\` themselves - with no ticket ID - will automate them directly, no TMS ticket required. This orchestrator stops here honestly rather than continuing into code generation, which it deliberately never triggers itself in any mode (see Purpose).
