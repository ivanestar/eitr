...
6. **Business-Intent & Criticality Analysis (Automatic, Strictly Read-Only):**
   ...
   - **Human Sign-Off Gateway:** present a Business-Intent Review Artifact table (Route, Business Feature, Criticality Tier, Confidence, Evidence Excerpt) for every new/changed entry - resolve each \`routeId\` to its \`docs/site-map/site-map.json\` path/title before printing, never show the raw \`routeId\`. If more than ~15 entries changed, show the highest-criticality/lowest-confidence rows first and summarize the rest as a count rather than listing every row. State explicitly: this file is NOT authoritative until a human has reviewed it - no other skill or agent should treat an entry with \`reviewed: false\` as ground truth.
