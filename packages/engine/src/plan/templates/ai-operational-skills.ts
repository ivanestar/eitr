...
6. **Business-Intent & Criticality Analysis (Automatic, Strictly Read-Only):**
   - Runs automatically for every active route, as part of both \`create\` and \`update\`, immediately after Step 4 - no separate request needed, unless the user explicitly asked to skip it (e.g. "just map the site, skip business-intent"):
     ...
     * **Core-Purpose Inference (app-level, once per crawl, after every route has been drafted above)**: synthesize 2-4 plausible one-sentence descriptions of the application's primary purpose, each grounded in evidence gathered across the whole crawl - the home route's heading/title carries the most weight, corroborated or contrasted by patterns across other routes' headings and nav labels - never invent a candidate with no evidence entry behind it. Mark the index of the candidate the evidence most strongly supports as \`mostLikelyIndex\`. Write \`corePurpose.candidates\` and \`corePurpose.mostLikelyIndex\` to \`docs/analysis/business-intent.json\` per \`.scaffold/schemas/business-intent.types.ts\`'s \`CorePurpose\` shape, \`reviewed: false\`.
     * **Core-Purpose Confirmation (a separate, lightweight exchange, before the Human Sign-Off Gateway below)**: present the candidates in conversation with the most-likely one marked as the recommended default, and ask the human to pick one or describe the application's purpose in their own words if none fit well. Use a structured interactive choice tool if your assistant provides one; otherwise plain numbered text with the recommended default clearly marked - the same convention \`/ground-zero-setup\`'s own mode-choice question already uses, since this renders across assistants with and without such a tool. Interpret the human's answer into \`corePurpose.selected\` (a \`Field<string>\`): picking an offered candidate carries that candidate's own evidence forward with \`source\` matching its strongest evidence signal; free text gets \`source: 'manual'\` and one \`evidence\` entry quoting the human's own words (still subject to the same <=100 char / PII-guard rule as every other evidence excerpt). Set \`corePurpose.reviewed: true\` and \`reviewedBy: 'human'\`.
...
1. **Pre-Flight Confirmation (mandatory, before anything runs):**
   ...
   * Present, in conversation, and wait for an explicit response before proceeding:
     - **What will run, in order**, starting from the current stage: \`/map-site create\` (full site crawl plus automatic business-intent inference), then \`/define-test-conditions\` (test analysis - defines test conditions for every reviewed route), then \`/design-test-cases\` (test design - test-level classification and a drafted test case per journey).
