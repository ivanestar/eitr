...
|---|---|---|
| \`artifacts/analysis/app-profile.json\` | Everything established about the application as a whole: what kind of application it is (production / sandbox-demo / internal tool), its confirmed core purpose, what each role is for, how it talks to its backend (\`apiStyle\`), the crawl boundary a human set, domain knowledge a person volunteered (\`domainNotes\`, including what they wrote under "Your notes" in a review, and the note they wrote on a single entry, with \`about\` naming it), the pages a person left out of testing (\`leftOutRoutes\` - never map or test one), which test types are in scope | \`node scripts/app-profile.mjs\` |
...
| \`artifacts/analysis/test-conditions.json\` | Per reviewed feature: what it is for, what every field means and should obey (with where that comes from), the research behind it, and its ranked test conditions - each with its layer (field, rule, behaviour, frame), where its expected result comes from (so whether it checks correctness or guards against regression), what it rests on, and its risk and priority. Every page that takes input has a main-flow condition naming where its result shows; every research check is cited by a condition or declined with a reason; every field rule the markup does not state is cited by what tests it or says why nothing does. \`artifacts/analysis/field-probes.json\` holds what fields did with typed values, \`artifacts/analysis/research/\` the research per kind of feature | \`node scripts/test-analysis-plan.mjs\` (where the stage is), \`node scripts/validate-test-conditions.mjs\`, \`node scripts/generate-test-conditions.mjs\` (builds and ranks) |
