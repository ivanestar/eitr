...
- **How much the triage is worth depends on who wrote it.** \`visualTriage.source\` is \`vision\` when a worker read the rendered page and \`heuristic\` when only a markup check ran; absent means neither did. A \`heuristic\` entry never claims more than \`ready\` plus flags, because guessing a page's state from tokens in its URL or from how few controls it has was measured wrong six times out of eight - read its \`flags\` as hints worth checking, never as findings.
- **\`redirectedFrom\` names the paths that land on this route.** A Page Object for it is also the Page Object a test reaches by requesting any of those, so do not scaffold a second one for a path listed there.
- **\`overlays\` is a list of dialogs someone has already found, opened and photographed for you.** Each entry names what raised it (\`trigger\` - \`auto\` for something every visitor gets, otherwise the control that opened it), what it contains (\`components\`), and, for the first route it appeared on, its own \`screenshot\`. Compose one as a \`Dialog\` child of the page that raises it, using that image for the icon-only controls the markup does not name, rather than re-discovering it by clicking around. An overlay whose \`trigger\` is \`auto\` is site furniture that will be in front of a test on arrival - model dismissing it, do not pretend it is not there. One recorded with \`dismissal.method: "gave-up"\` was never actually cleared, so treat that route's other findings as partial.
