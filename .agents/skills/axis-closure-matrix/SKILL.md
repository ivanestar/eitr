...
- A stat-row summarizing counts: confirmed bugs, Core gaps, Extended/OK-as-is, already-correct
  ...
  table itself.
- The Core/Extended table itself, using the 5-column schema from step 2, with badge tags for
  Core/Extended and color-coded status (green/amber/red/grey) for the status column — reuse the
  color tokens already established in this repo's prior axis reports.
