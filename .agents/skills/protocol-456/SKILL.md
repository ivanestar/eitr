...
- Verify target files fresh with `view_file` (Context Freshness - never from memory).
...
- **On-Demand Web Research**: When researching third-party library behaviors, breaking changes, or modern best practices, perform targeted searches via `search_web` against official documentation (e.g. `playwright.dev`, `nodejs.org`, `github.com`, `vitest.dev`). Focus on high-quality findings, pitfalls, and concrete code patterns.
...
- Phase 3b (GREEN): Implement the minimal code edit using `replace_file_content`. Run target test until 100% green.
...
- Execute git commit strictly via `npm run commit -- "<message>"` to enforce mandatory evening 23:00 OpSec timestamps per AGENTS.md Section 5.
