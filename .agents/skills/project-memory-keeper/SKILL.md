...
Use the `replace_file_content` or `multi_replace_file_content` tools to update the sections in `AI_MEMORY.md` rather than rewriting the entire file from scratch.
...
1. Use `view_file` to read `AI_MEMORY.md`.
...
**Good Example 1:**
User: "save progress"
Action: Agent uses `replace_file_content` to add 1 bullet point under "Recently Completed" in `AI_MEMORY.md`. Agent outputs exactly 1 sentence acknowledging the save.
...
_(Violation: Did not use replace_file_content for targeted update)._
